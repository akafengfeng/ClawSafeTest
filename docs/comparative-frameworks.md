...
ClawSafe also adopts Progent's **automation**, with one deliberate design choice: **the LLM assists testing and policy authoring, but never sits in the guard's runtime.** The protection path is pure-Python and calls no LLM (enforced by `tests/test_runtime_llm_free.py`). [`PolicyGenerator`](https://github.com/akafengfeng/ClawSafeTest/blob/main/clawsafe/core/policy_generation.py) is an authoring aid — an LLM *drafts* least-privilege rules that are sanitized, reviewed, and committed as static JSON, which the deterministic engine then enforces. Separately, an [LLM red-teamer](https://github.com/akafengfeng/ClawSafeTest/blob/main/benchmarks/redteam.py) generates adversarial scenarios to *test* the guard, and an [LLM-as-judge](https://github.com/akafengfeng/ClawSafeTest/blob/main/benchmarks/judge.py) grades L3 outcomes semantically (did the agent actually do harm?) and filters generated attacks for quality — again, evaluation-only, never in the protection path (a runtime judge would itself be an injection target, since the content it judges is attacker-controlled). The security-critical difference from a naive implementation is that generated policy is treated as **untrusted input, not authority**:
