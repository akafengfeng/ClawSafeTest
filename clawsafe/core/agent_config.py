...
class AgentGuardConfig:
    ...
    Attributes:
        ...
        enable_rate_limiting: Enable per-tool sliding-window rate limits.

    Rate limits are per-tool (see :class:`~clawsafe.core.tools.ToolPolicy`);
    there is no global quota. Execution timeouts and memory limits are not
    enforced by this layer — run tools in your own sandbox for that.
    """
