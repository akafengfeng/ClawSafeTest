...
- **One package, two tiers**: the base install is now the zero-dependency lite
  ...
  `clawsafe.full` namespace. Both tiers are zero-dependency — the guard
  framework never calls an LLM. Vendor SDKs moved behind the `[providers]`
  extra (`[full]` kept as an alias), needed only for proxy mode where
  `ClawSafeAgent` makes the LLM calls; `anthropic` is no longer a hard
  dependency. Existing imports (`from clawsafe import AgentGuard`) keep
  working unchanged.
